/**
 * Vynaro v2.5.0 · 设置页 (M3.2 接通后端 · M4.5 Locale 切换)
 *
    ...
    <div className="mx-auto max-w-3xl space-y-8 px-8 py-10">
      <header className="flex items-start justify-between">
        <div className="space-y-1">
          ...
          <h1 className="text-3xl font-extrabold tracking-tight text-[var(--color-text-primary)]">系统设置</h1>
          <p className="text-sm text-[var(--color-text-secondary)]">
            {isLoading ? "加载中..." : "配置 LLM 大模型 / TTS 语音合成 / 界面主题与语言"}
          </p>
        </div>
        {saved && (
          <span className="inline-flex items-center gap-2 rounded-full border border-emerald-500/40 bg-emerald-500/10 px-3.5 py-1 text-xs font-medium text-emerald-400">
            ✓ 设置已保存
          </span>
        )}
      </header>
      ...
      {/* TTS */}
      <Section title="🎙️ 语音合成" subtitle="第一人称独白叙述的配音引擎">
      ...
      {/* Appearance */}
      <Section title="🎨 外观与语言 (Appearance & Language)">
