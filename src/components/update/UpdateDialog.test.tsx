/**
 * splicr v1.0.0 · UpdateDialog 组件单测
 *
 * 覆盖:
...
describe("UpdateDialog", () => {
  ...
  it("idle 状态:显示 '检查更新' 主按钮 + 重置隐藏", () => {
    ...
    expect(screen.getByText(/splicr 应用更新器/)).toBeInTheDocument();
  ...
  it("ready 状态:显示完成图标 + '重启安装' + 下载路径", () => {
    mockSnapshot = {
      ...
      downloadedPath: "/cache/splicr-update-3.1.0.zip",
    };
    render(<UpdateDialog open={true} onClose={() => {}} />, {
      wrapper: makeWrapper(),
    });
    ...
    expect(
      screen.getByText("/cache/splicr-update-3.1.0.zip"),
    ).toBeInTheDocument();
  });
