...
from app.ui.main.tray_manager import get_tray_manager
...
class SystemTrayController(QObject):
    ...
    def set_minimize_to_tray(self, enabled: bool) -> None:
        ...
        if enabled and not tray.is_enabled:
            tray.enable(self._window_title)
        elif not enabled and tray.is_enabled:
            tray.disable()

    def set_window_title(self, title: str) -> None:
        """Update the window title used for tray tooltips and menu headers."""
        self._window_title = title
        tray = self._tray
        if tray is not None and tray.is_enabled:
            tray.retranslate(title)

    def retranslate(self) -> None:
        """Refresh tray text after the active language changes."""
        tray = self._tray
        if tray is not None and tray.is_enabled:
            tray.retranslate(self._window_title)
        ...
        if (
            ...
            if not self._tray_hint_shown:
                tray.show_notification(
                    t("tray.notification.title").format(
                        name=self._window_title),
                    t("tray.notification.minimized"),
                )
