...
class Colors:
    ...
    # Phase 2 · 图表网格线（浅色用低对比灰）
    GRID_LINE = "rgba(15,141,168,0.08)"
...
class DarkColors:
    """Dark theme palette — "tech-dark" variant (Phase 2).

    Mirror of :class:`Colors` with values tuned for the low-light,
    neon-accented workbench aesthetic. Only the colour tokens are
    redefined here — ``Radii`` / ``FontSizes`` / ``Shadows`` etc. are
    shared with the light theme because they are theme-agnostic.

    Phase 2 调整要点：

    * 更深的底色（``#0a0e1a`` vs 之前 ``#0f172a``）
    * 新增 :py:attr:`BG_GLASS` / :py:attr:`GRID_LINE` 等数据可视化相关 token
    * 新增三个贡虹色 token：:py:attr:`NEON_CYAN` / :py:attr:`NEON_MAGENTA` /
      :py:attr:`NEON_GREEN`
    * :py:attr:`PRIMARY` 调整为更亮的贡虹青，作为 Dashboard 强调色
    """

    # ── 背景层 ──────────────────────────────────────────────────
    BG_BASE = "#0a0e1a"        # 深蓝基底
    BG_SURFACE = "#111827"     # 面板表面
    BG_ELEVATED = "#1f2937"
    BG_OVERLAY = "#374151"
    BG_INPUT = "#0f172a"
    BG_GLASS = "rgba(17,24,39,0.6)"   # 毛玻璃背景
    GRID_LINE = "rgba(34,211,238,0.08)"  # 图表网格线
...
class Shadows:
    ...
    # Phase 2 · 贡虹辉光（深色主题下 Dashboard 卡片 hover 表现）
    GLOW_CYAN = "0 0 0 1px rgba(34,211,238,0.35), 0 0 24px rgba(34,211,238,0.35)"
    GLOW_MAGENTA = "0 0 0 1px rgba(217,70,239,0.35), 0 0 20px rgba(217,70,239,0.30)"
...
class QSSComponents:
    ...
    def glass_card() -> str:
        ...
        背景使用 :py:attr:`_C.BG_GLASS`（半透明），边框使用
        :py:attr:`_C.PRIMARY`（贡虹青），hover 时切换为
        :py:attr:`_C.NEON_CYAN` 加 :py:attr:`Shadows.GLOW_CYAN`。
        """
        return f"""
            QFrame#glass_card {{
                background: {_C.BG_GLASS};
                border: 1px solid {_C.PRIMARY}33;
    ...
    def combobox() -> str:
        ...
        def _chevron(color: str) -> str:
            svg = (
                ...
                f'<path d="M3 4.5L6 7.5L9 4.5" stroke="{color}" '
        ...
        return f"""
            ...
            QComboBox::down-arrow {{
                image: url({_chevron(muted)});
                width: 12px;
                height: 12px;
            }}
            QComboBox:hover::down-arrow {{
                image: url({_chevron(strong)});
            }}
