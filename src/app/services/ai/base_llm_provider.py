...
class HTTPClientMixin:
    ...
    async def _call_api_with_retry(
        ...
        if getattr(self, "_retry_handler", None) is not None:
            handler = self._retry_handler
            if hasattr(handler, "execute") and callable(handler.execute):
                return await handler.execute(_call)
            elif callable(handler):
                return await handler(_call)

        return await retry_async(_call, max_attempts=self._retry_max_attempts, delay=self._retry_base_delay)
