...
if TYPE_CHECKING:
    from app.services.ai.script_generator.script_generator import ScriptGenerator
...
def understand_step(ctx: NarrationContext) -> StepResult:
    ...
    if ctx.short_drama_style is not None:
        try:
            from app.pipeline.short_drama import (
                ShortDramaNarrator,
                ShortDramaPreset,
            )
...
def storygraph_step(ctx: NarrationContext) -> StepResult:
    ...
    try:
        from app.services.video_understanding import LongVideoUnderstanding
        from app.services.video_understanding.models import UnderstandingLevel
