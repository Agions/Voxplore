...
from app.api.schemas.models import NarrationRequest, PipelineStatus
...
async def create_narration_task(request: NarrationRequest):
    ...
    t = asyncio.create_task(_process_narration(task_id))
    t.add_done_callback(lambda task_obj: _handle_task_done(task_obj, task_id))

    return {"task_id": task_id, "status": "pending"}


def _handle_task_done(task_obj: asyncio.Task, task_id: str) -> None:
    """后台任务异常捕捉回调"""
    if task_obj.cancelled():
        _task_store.update(task_id, status="cancelled", current_step="已取消")
    elif task_obj.exception():
        exc = task_obj.exception()
        logger.error(f"Task {task_id} unhandled exception: {exc}")
        _task_store.update(
            task_id, status="error", error=str(exc), current_step="处理异常"
        )
...
@router.post("/pipeline/{task_id}/cancel", status_code=202)
@router.get("/pipeline/{task_id}/cancel", status_code=202, deprecated=True)
async def cancel_task(task_id: str):
    """取消任务（推荐使用 POST，GET 保持向后兼容）"""
    task = _task_store.get(task_id)
    if task is None:
        raise HTTPException(status_code=404, detail="Task not found")
...
async def _process_narration(task_id: str):
    ...
    try:
        ...
        scene_info = integrator.analyze_scenes(project)
        project.scene_info = scene_info
