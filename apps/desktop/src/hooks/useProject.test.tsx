/**
 * SceneFab v2.5.0 · useProject hook 单测 (renderHook)
 *
 * 测试策略:
...
describe("useProject", () => {
  ...
  describe("open", () => {
    it("成功加载 → setCurrentRecord 被调,sync 到 query cache", async () => {
      ...
      await act(async () => {
        await result.current.open("/tmp/test.scenefab.json");
      });

      expect(projectLoadMock).toHaveBeenCalledWith("/tmp/test.scenefab.json");
      expect(storeState.setCurrentRecord).toHaveBeenCalledWith(
        "/tmp/test.scenefab.json",
  ...
  describe("save", () => {
    ...
    it("正常 save → 写 updated_at 并同步 store", async () => {
      storeState.current = sampleProject;
      storeState.currentPath = "/tmp/test.scenefab.json";
