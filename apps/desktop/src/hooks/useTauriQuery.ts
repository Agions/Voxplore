/**
 * SceneFab v2.5.0 · useTauriQuery — React Query + Tauri Invoke 通用封装
 *
 * 自动处理:
 * - Query Key 命名空间 (避免与其它 query 冲突)
 * - 错误抛出 SceneFabError (可在 onError 中统一 toast)
...
export interface UseTauriQueryOptions<C extends IpcCommand> extends Omit<
  UseQueryOptions<CommandResultOf<C>, SceneFabError, CommandResultOf<C>>,
  "queryKey" | "queryFn"
> {
  command: C;
...
): UseQueryResult<CommandResultOf<C>, SceneFabError> {
  const { command, args, queryKeyPrefix, ...rest } = options;
  return useQuery<CommandResultOf<C>, SceneFabError, CommandResultOf<C>>({
    ...
    queryFn: async () => {
      try {
        ...
        if (
          ...
          throw err as SceneFabError;
        }
        throw {
          kind: "other",
          message: err instanceof Error ? err.message : String(err),
        } satisfies SceneFabError;
      }
    },
    retry: false,
    staleTime: 5_000,
