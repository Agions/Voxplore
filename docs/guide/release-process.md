---
title: 发布流程
description: Monoloop v1.0.0 桌面端的 GitHub Releases 发布、签名、CI 集成指引。
---

# 发布流程

本文档描述 Monoloop v1.0.0 桌面端（Tauri 2 + Rust + React 19）的工程化发布流程，覆盖：

- 版本号与 `Cargo.toml` / `tauri.conf.json` 同步
- `tauri build` 三平台产物
- GitHub Releases 上传
- 应用更新器（M4 monoloop-update / M5 tauri-plugin-updater）配套策略
...
| 平台    | 路径                                                                                                 |
| ------- | ---------------------------------------------------------------------------------------------------- |
| macOS   | `apps/desktop/src-tauri/target/universal-apple-darwin/release/bundle/macos/Monoloop.app`             |
| macOS   | `apps/desktop/src-tauri/target/universal-apple-darwin/release/bundle/dmg/Monoloop_*.dmg`             |
| Windows | `apps/desktop/src-tauri/target/x86_64-pc-windows-msvc/release/bundle/msi/Monoloop_*.msi`             |
| Linux   | `apps/desktop/src-tauri/target/x86_64-unknown-linux-gnu/release/bundle/deb/Monoloop_*.deb`           |
| Linux   | `apps/desktop/src-tauri/target/x86_64-unknown-linux-gnu/release/bundle/appimage/Monoloop_*.AppImage` |
...
2. `pnpm --filter monoloop-desktop build` （前端）
...
release 标题必须以 `v` 开头（`monoloop-update` 内部 `tag_name.trim_start_matches('v')` 已兼容，但 GitHub webhook 触发器仍要求 `v` 前缀）。

### 3.1 update 资源元数据

为了让 `monoloop-update` 自动探测 SHA-256 校验，二进制 asset 上传时 GitHub 仓库需要启用 **Generate release notes** 的同名 `.sha256` 文件：

```bash
# 上传前生成
shasum -a 256 Monoloop_2.5.0-beta.1_aarch64.dmg > Monoloop_2.5.0-beta.1_aarch64.dmg.sha256
gh release upload v1.0.0 -beta.1 Monoloop_2.5.0-beta.1_aarch64.dmg.sha256
```

或者更优的做法 — 把 SHA-256 直接写入 `Asset.digest`（GitHub 在 web UI 自动生成），`monoloop-update` 会读取：

```json
{
  "name": "Monoloop_2.5.0-beta.1_aarch64.dmg",
  "digest": "sha256:abc123...",
  "browser_download_url": "https://github.com/.../Monoloop_2.5.0-beta.1_aarch64.dmg"
}
```

## 4. 应用更新器策略

### 4.1 M4 阶段（当前）— `monoloop-update` crate

- 探测：`https://api.github.com/repos/qingshanyanyu/Monoloop/releases/latest`
...
.register(Arc::new(UpdateService::new(
    ...
    "qingshanyanyu/Monoloop",  // TODO 配置文件化
)))
```
...
| ④    | `monoloop-update` 内部用 `tauri::updater::Updater` 替代 `reqwest` 直链   |
| ⑤    | 自动重启安装（替代 M4 手动下载目录）                                     |
...
cargo tauri signer sign --private-key $TAURI_SIGNING_PRIVATE_KEY \
  apps/desktop/src-tauri/target/release/bundle/macos/Monoloop.app.tar.gz
```
