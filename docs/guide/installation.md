---
title: 安装指南
description: 在 Windows、macOS 和 Linux 上完整安装 Monoloop v1.0.0 桌面应用（Tauri 2 + React 主线）。
---
...
1. 从 [Releases](https://github.com/Agions/monoloop/releases) 下载最新 `.msi` 或 `.exe`
2. 双击运行安装程序
3. 安装完成后，桌面会出现 **Monoloop** 图标，从开始菜单启动
...
1. 从 [Releases](https://github.com/Agions/monoloop/releases) 下载最新 `.dmg`
2. 双击挂载，将 `Monoloop.app` 拖入「应用程序」文件夹
3. 启动台中搜索 **Monoloop** 启动
...
1. 从 [Releases](https://github.com/Agions/monoloop/releases) 下载最新 `.deb` 或 `.AppImage`
2. **deb**：`sudo dpkg -i monoloop_2.5.0_amd64.deb && sudo apt install -f`
3. **AppImage**：`chmod +x Monoloop_2.5.0_amd64.AppImage && ./Monoloop_2.5.0_amd64.AppImage`
...
# 成功标志：弹出 Tauri 窗口 + 看到 Monoloop Home 页面
```
