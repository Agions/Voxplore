...
use monoloop_core::domain::Project;
use monoloop_core::services::ProjectService;
...
#[tauri::command]
pub async fn project_create_blank(
    app: tauri::AppHandle,
    ...
    tokio::fs::create_dir_all(&dir)
        .await
        .map_err(|e| e.to_string())?;
    let path = dir.join(format!("{}.monoloop.json", proj.id));
...
#[tauri::command]
pub async fn project_add_media(
    path: String,
    project: Project,
    media: MediaFile,
...
async fn write_project(path: &Path, proj: &Project) -> Result<(), std::io::Error> {
    ...
    let bytes = serde_json::to_vec_pretty(proj)
