...
//! - `assets_thumbnail`   : ffmpeg 抽首帧 JPEG,落盘到 `~/.cache/scenefab/thumb/`
//! - `assets_search`      : 在项目已有 media_files 中按 name/path substring 过滤
//!
//! ## 安全
//! - 所有 IO 失败统一 `SceneFabError::Io`
...
use scenefab_ffmpeg::{parse_probe_json, Ffmpeg, FfmpegProbe};
...
pub enum AssetError {
    ...
    #[error("scenefab 错误: {0}")]
    Core(#[from] scenefab_core::error::SceneFabError),
}
...
fn thumb_cache_dir() -> AssetResult<PathBuf> {
    ...
    let dir = base.join("scenefab").join("thumb");
