...
pub use scenefab_core::error::{LlmProviderKind, SceneFabError, SceneFabResult};
...
impl LlmProvider for OpenAiCompatible {
    ...
    async fn chat(&self, req: &LlmRequest) -> SceneFabResult<LlmResponse> {
        ...
        let api: ApiResp = resp.json().await.map_err(|e| SceneFabError::Llm {
            provider: self.kind,
...
impl LlmManager {
    ...
    pub async fn chat(&self, req: &LlmRequest) -> SceneFabResult<LlmResponse> {
        ...
        let mut last_err: Option<SceneFabError> = None;
        ...
        Err(last_err.unwrap_or_else(|| {
            SceneFabError::Other("LlmManager: chain empty after iteration".into())
        }))
    }
}
