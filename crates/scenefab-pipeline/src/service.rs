//! scenefab-pipeline · PipelineService (运行时编排门面)
//!
//! ## 职责
//! - 持有 [`Pipeline`] 状态机 + 当前项目 + 后台任务句柄
//! - `start()` 把 5 步流水线 spawn 到 tokio 后台任务,命令层立即返回
...
use crate::{Pipeline, PipelineEvent, PipelineState, Project, SceneFabResult, StepStatus};
...
impl PipelineService {
    ...
    pub async fn status(&self) -> PipelineStatus {
        ...
        PipelineStatus {
            ...
            step_statuses: self.pipeline.step_statuses().to_vec(),
            current_step: self.pipeline.current_step(),
            project_name,
        }
    }

    /// 启动流水线 (后台任务)。运行中重复调用返回错误。
    pub async fn start(&self, project: Project, deps: Arc<PipelineDeps>) -> SceneFabResult<()> {
        if self.is_running() {
            return Err(crate::SceneFabError::Pipeline("流水线正在运行中".into()));
        }
