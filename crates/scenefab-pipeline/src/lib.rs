...
//! - ④ 配音字幕 (MonologueMaker.generate_voice + generate_captions)
//! - ⑤ 导出发布 (export draft + close pipeline)
//!
//! ## M2 范围 (PoC)
...
/// 5 步常量 (与 `production_viewmodel.STEP_DEFINITIONS` 1:1)。
pub const STEPS: [StepDef; 5] = [
    StepDef {
        id: "01",
        label_zh: "素材导入",
        description_zh: "选择影视片段并记录来源",
    },
    StepDef {
        id: "02",
        label_zh: "场景拆分",
        description_zh: "识别人物、冲突和关键转折",
    },
    StepDef {
        id: "03",
        label_zh: "脚本生成",
        description_zh: "使用第一人称视角组织叙事",
    },
    StepDef {
        id: "04",
        label_zh: "配音字幕",
        description_zh: "完成音频、字幕与节奏校准",
    },
    StepDef {
        id: "05",
        label_zh: "导出发布",
        description_zh: "按竖屏平台参数生成成片",
    },
];
...
pub trait StepExecutor: Send + Sync {
    ...
    /// 执行一步 (副作用在内部完成)
    async fn execute(&self, project: &mut Project) -> SceneFabResult<()>;
}
...
impl std::fmt::Debug for Pipeline {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        ...
        f.debug_struct("Pipeline")
            .field("state", &st.pipeline_state)
            .field("current_step", &st.current_step)
            .field("step_statuses", &st.step_statuses)
            .finish()
    }
}
...
struct PipelineInner {
    ...
    step_statuses: [StepStatus; 5],
    current_step: i8, // -1 = 没起步,0..=4 = 正在跑某步
...
impl Default for PipelineInner {
    fn default() -> Self {
        Self {
            ...
            step_statuses: [StepStatus::Pending; 5],
            current_step: -1,
            started_at: None,
        }
    }
...
impl Pipeline {
    pub fn new() -> Self {
        ...
        Self {
            state: parking_lot::Mutex::new(PipelineInner::default()),
            events_tx: tx,
    ...
    pub fn step_statuses(&self) -> [StepStatus; 5] {
    ...
    pub async fn start(
    ...
    ) -> SceneFabResult<()> {
        {
            ...
            // reset
            st.step_statuses = [StepStatus::Pending; 5];
            st.current_step = -1;
        ...
        for (idx, exec) in executors.iter().enumerate() {
            ...
            if self.cancel_requested.load(Ordering::SeqCst) {
                ...
                return Err(SceneFabError::Pipeline(msg));
            }

            if exec.step_index() as usize != idx {
                return Err(SceneFabError::Pipeline(format!(
            ...
            match result {
                Ok(()) => {
                    ...
                    let _ = self.events_tx.send(PipelineEvent::StepFinished {
                        index: idx as u8,
                        duration_ms,
                    });
                }
                Err(e) => {
                    ...
                    let msg = e.to_string();
                    let _ = self.events_tx.send(PipelineEvent::StepFailed {
                        index: idx as u8,
                        message: msg.clone(),
                    });
                    let _ = self
        ...
        let _ = self.events_tx.send(PipelineEvent::PipelineFinished {
            project_path: project.id.clone(),
        });
        Ok(())
    }
...
mod tests {
    ...
    impl StepExecutor for AlwaysOkExecutor {
        ...
        async fn execute(&self, _p: &mut Project) -> SceneFabResult<()> {
            Ok(())
        }
    }
    ...
    impl StepExecutor for AlwaysFailExecutor {
        ...
        async fn execute(&self, _p: &mut Project) -> SceneFabResult<()> {
            Err(SceneFabError::Pipeline("intentional fail".into()))
        }
    }
    ...
    async fn run_all_5_steps_ok() {
        ...
        let execs: Vec<Box<dyn StepExecutor>> = (0..5)
            .map(|i| Box::new(AlwaysOkExecutor { idx: i }) as Box<dyn StepExecutor>)
            .collect();
    ...
    async fn step_failure_aborts() {
        let p = Pipeline::new();
        let mut proj = Project::default();
        let fails: Vec<Box<dyn StepExecutor>> = (0..5)
            .map(|i| Box::new(AlwaysFailExecutor { idx: i }) as Box<dyn StepExecutor>)
            .collect();
    ...
    async fn cancel_before_next_step() {
        ...
        let execs: Vec<Box<dyn StepExecutor>> = vec![
            Box::new(AlwaysOkExecutor { idx: 0 }),
            Box::new(CancelAfterFirst { p: p.clone() }),
            Box::new(AlwaysOkExecutor { idx: 2 }),
            Box::new(AlwaysOkExecutor { idx: 3 }),
            Box::new(AlwaysOkExecutor { idx: 4 }),
        ];
