...
use scenefab_domain::{ExportRecord, Project, ScriptSegment};
use scenefab_ffmpeg::Ffmpeg;
use scenefab_llm::{LlmProvider, LlmRequest};
use scenefab_tts::{TtsEngine, TtsRequest};

use crate::{SceneFabError, SceneFabResult, StepExecutor};
...
/// 构造标准 5 步执行器列表 (与 STEPS 常量 1:1)。
pub fn default_executors(deps: Arc<PipelineDeps>) -> Vec<Box<dyn StepExecutor>> {
    vec![
        Box::new(IngestStep { deps: deps.clone() }),
        Box::new(SceneSplitStep { deps: deps.clone() }),
        Box::new(ScriptGenStep { deps: deps.clone() }),
        Box::new(VoiceCaptionsStep { deps: deps.clone() }),
        Box::new(ExportStep { deps }),
    ]
}
...
impl StepExecutor for IngestStep {
    ...
    async fn execute(&self, project: &mut Project) -> SceneFabResult<()> {
        if project.media_files.is_empty() {
            return Err(SceneFabError::Project(
        ...
        for mf in project.media_files.iter_mut() {
            if !mf.path.exists() {
                return Err(SceneFabError::Project(format!(
                    "素材文件不存在: {}",
                    mf.path.display()
                )));
            }
...
impl StepExecutor for ScriptGenStep {
    ...
    async fn execute(&self, project: &mut Project) -> SceneFabResult<()> {
        let llm = self.deps.llm.as_ref().ok_or_else(|| {
            SceneFabError::Config("未配置 LLM 提供商或 API 密钥,请先到设置页填写".into())
        })?;
...
impl StepExecutor for VoiceCaptionsStep {
    ...
    async fn execute(&self, project: &mut Project) -> SceneFabResult<()> {
        let tts = self.deps.tts.as_ref().ok_or_else(|| {
            SceneFabError::Config("未配置 TTS 引擎,请先到设置页选择配音引擎".into())
        })?;
        ...
        // SRT 字幕落盘
        let srt = scenefab_ffmpeg::build_srt(&srt_entries);
...
mod tests {
    ...
    fn project_with_media() -> Project {
        ...
        p.media_files.push(MediaFile {
            path: PathBuf::from("/tmp/scenefab-test-dummy.mp4"),
