//! scenefab-tts v2.5.0 · TTS 引擎层 (M3.2 完整实现)
//!
//! ## 责任范围
//! 3 个 TTS 引擎的统一抽象与实现:
...
use scenefab_core::error::{SceneFabError, SceneFabResult};
use tokio_tungstenite::tungstenite::Message;

// 同时保留两个常用名称,方便业务侧 import 习惯
pub use scenefab_core::error::TtsProviderKind;
pub use scenefab_core::error::TtsProviderKind as ProviderKind;
...
pub trait TtsEngine: Send + Sync {
    ...
    /// 合成并落盘。失败返回 `SceneFabError::Tts`。
    async fn synthesize(&self, req: &TtsRequest) -> SceneFabResult<TtsOutcome>;
}

fn tts_err(provider: TtsProviderKind, message: impl Into<String>) -> SceneFabError {
    SceneFabError::Tts {
        provider,
...
/// 落盘 helper: 自动建父目录。
async fn write_output(path: &Path, bytes: &[u8]) -> SceneFabResult<u64> {
