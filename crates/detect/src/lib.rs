//! monoloop-ffmpeg v1.0.0  · FFmpeg 包装层 (M3.2 完整实现)
//!
//! ## 责任范围
...
//! - 失败统一 `MonoloopError::Ffmpeg(stderr tail)`
...
impl Ffmpeg {
    ...
    pub fn discover() -> MonoloopResult<Self> {
        let ffmpeg = which("ffmpeg")
            .ok_or_else(|| MonoloopError::Ffmpeg("ffmpeg 不在 PATH 中 (请先安装 ffmpeg)".into()))?;
        let ffprobe = which("ffprobe").ok_or_else(|| {
            MonoloopError::Ffmpeg("ffprobe 不在 PATH 中 (请先安装 ffmpeg)".into())
        })?;
        Ok(Self {
            ffmpeg_bin: ffmpeg,
            ffprobe_bin: ffprobe,
    ...
    pub async fn probe(&self, path: &Path) -> MonoloopResult<FfmpegProbe> {
        if !path.exists() {
            return Err(MonoloopError::Ffmpeg(format!(
                "文件不存在: {}",
                path.display()
            )));
        }
    ...
    pub async fn extract_audio(&self, input: &Path, output: &Path) -> MonoloopResult<()> {
        self.run_encode(
            &[
                "-i",
                &input.to_string_lossy(),
    ...
    pub async fn scale_pad_vertical(
        &self,
        input: &Path,
        output: &Path,
        width: u32,
    ...
    pub async fn concat(&self, inputs: &[PathBuf], output: &Path) -> MonoloopResult<()> {
        if inputs.is_empty() {
            return Err(MonoloopError::Ffmpeg("concat: 输入列表为空".into()));
        }
    ...
    async fn run_raw(
        ...
        let mut child = cmd.spawn().map_err(|e| RawExecError {
        ...
        let stdout_task = async move {
            ...
            if let Some(mut out) = stdout_take {
                out.read_to_end(&mut buf).await.map_err(|e| RawExecError {
                    stderr_tail: format!("read stdout: {e}"),
                })?;
            }
        ...
        let status = child.wait().await.map_err(|e| RawExecError {
            stderr_tail: format!("wait: {e}"),
        })?;
...
impl From<RawExecError> for MonoloopError {
    fn from(e: RawExecError) -> Self {
        MonoloopError::Ffmpeg(e.stderr_tail)
    }
}
...
pub fn parse_probe_json(json: &str) -> MonoloopResult<FfmpegProbe> {
    let v: serde_json::Value = serde_json::from_str(json)
        .map_err(|e| MonoloopError::Ffmpeg(format!("ffprobe JSON 解析失败: {e}")))?;
