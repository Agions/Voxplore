...
/// - 仓库 / tag 模板可配置(默认 `qingshanyanyu/SceneFab`)
...
impl UpdateService {
    ...
    pub fn new(current_version: impl Into<String>, repo: impl Into<String>) -> Self {
        ...
        Self {
            ...
            client: reqwest::Client::builder()
                ...
                .user_agent("scenefab-updater/3.0")
                .build()
    ...
    pub async fn check(&self) -> Result<UpdateInfo, UpdateError> {
        ...
        let binary = rel.assets.iter().find(|a| {
            a.digest
                .as_deref()
    ...
    /// 下载 latest 到 `dest`(可显式传;否则落到 std::env::temp_dir()/scenefab-update-<version>)
    pub async fn download(&self, dest: Option<PathBuf>) -> Result<PathBuf, UpdateError> {
        ...
        let dest = dest.unwrap_or_else(|| {
            std::env::temp_dir().join(format!("scenefab-update-{}.bin", info.version))
        });

        {
            ...
            s.progress = Some(UpdateProgress {
                downloaded_bytes: 0,
                total_bytes: info.file_size_bytes.unwrap_or(0),
                percent: 0,
            });
            s.error = None;
        }
        ...
        let total = resp
            .content_length()
            .unwrap_or(info.file_size_bytes.unwrap_or(0));
...
mod tests {
    ...
    fn phase_state_default_is_idle() {
        let svc = UpdateService::new("3.0.0-alpha.0", "qingshanyanyu/SceneFab");
    ...
    async fn check_handles_http_error_gracefully() {
        ...
        let svc = UpdateService::new("3.0.0-alpha.0", "qingshanyanyu/this-repo-does-not-exist-zzz");
