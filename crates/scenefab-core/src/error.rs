...
/// LLM 提供商标识 (与 `scenefab-llm::LlmManager` 中定义的 11 个 provider 一一对应)。
...
// 把 SceneFabError 序列化为 JSON 对象给 Tauri 前端
impl Serialize for SceneFabError {
    fn serialize<S: serde::Serializer>(&self, s: S) -> Result<S::Ok, S::Error> {
        ...
        let mut st = s.serialize_struct("SceneFabError", 2)?;
