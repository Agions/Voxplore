//! scenefab-core · 默认 19 个核心服务的注册与实现
//!
//! ## M2 已落地 (3 个)
...
impl ConfigService {
    ...
    pub async fn snapshot(&self) -> ConfigSnapshot {
        self.inner
            ...
            .await
            .clone()
            .unwrap_or_else(ConfigSnapshot::default)
    }
...
pub async fn register_default_services(container: &ServiceContainer) {
    container
        .register::<ProjectService>(Arc::new(ProjectService::new()))
        .await;
    container
        .register::<ConfigService>(Arc::new(ConfigService::new()))
        .await;
    container
    ...
    tracing::info!(
        "scenefab-core: 3/{total} default services registered (M2 milestone)",
        total = 19
    );
}
...
mod tests {
    ...
    async fn project_service_recent_works() {
        ...
        s.push_recent(PathBuf::from("/tmp/a.scenefab")).await;
        s.push_recent(PathBuf::from("/tmp/b.scenefab")).await;
        s.push_recent(PathBuf::from("/tmp/a.scenefab")).await; // 重复会移到首位
        let r = s.recent_projects().await;
        assert_eq!(r.len(), 2);
        assert_eq!(r[0], PathBuf::from("/tmp/a.scenefab"));
    }
