...
//! - [`error`]     : `SceneFabError` + `SceneFabResult` — 统一错误类型 (Tauri Command 返回)
//! - [`logging`]   : `init_logging` — `tracing_subscriber` 初始化
//! - [`domain`]    : re-export 跨模块领域原语 (delegate 给 scenefab-domain)
...
//! - `SceneFabError` 共 9 类 (`Config`/`Llm`/`Tts`/`Ffmpeg`/`Io`/`Project`/`Pipeline`/`Plugin`/`Updater`)
