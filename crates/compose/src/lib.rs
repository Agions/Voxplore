//! monoloop-pipeline v1.0.0  · 7 步第一人称叙述流水线
//!
//! ## 流水线定义 (7 步状态机)
...
pub use monoloop_core::error::{MonoloopError, MonoloopResult};
pub use monoloop_domain::{Clip, MediaFile, Project, ScriptSegment, Timeline, Track};
...
pub const STEPS: [StepDef; 7] = [
    StepDef {
        id: "intake",
        label_zh: "素材导入",
        description_zh: "选择影视片段并导入项目",
    },
    StepDef {
        id: "detect",
        label_zh: "智能拆条",
        description_zh: "FFmpeg 识别镜头切换与关键帧",
    },
    StepDef {
        id: "script",
        label_zh: "脚本生成",
        description_zh: "LLM 生成第一人称解说独白",
    },
    StepDef {
        id: "voice",
        label_zh: "TTS配音",
        description_zh: "语音合成与情绪节奏校准",
    },
    StepDef {
        id: "subtitle",
        label_zh: "字幕合成",
        description_zh: "VAD 对齐字幕时间轴并渲染",
    },
    StepDef {
        id: "compose",
        label_zh: "画面对齐",
        description_zh: "音画合成与竖屏帧率压制",
    },
    StepDef {
        id: "export",
        label_zh: "导出发布",
        description_zh: "按平台参数封装输出成片",
    },
];
...
pub trait StepExecutor: Send + Sync {
    ...
    /// 执行一步 (副作用在内部完成)
    async fn execute(&self, project: &mut Project) -> MonoloopResult<()>;
}
...
impl Default for PipelineInner {
    fn default() -> Self {
        Self {
            ...
            step_statuses: [StepStatus::Pending; 7],
            current_step: -1,
            started_at: None,
        }
    }
    ...
    pub async fn start(
        ...
        for (idx, exec) in executors.iter().enumerate() {
            ...
            if self.cancel_requested.load(Ordering::SeqCst) {
                ...
                return Err(MonoloopError::Pipeline(msg));
            }

            if exec.step_index() as usize != idx {
                return Err(MonoloopError::Pipeline(format!(
...
mod tests {
    ...
    impl StepExecutor for AlwaysOkExecutor {
        ...
        async fn execute(&self, _p: &mut Project) -> MonoloopResult<()> {
            Ok(())
        }
    }
    ...
    impl StepExecutor for AlwaysFailExecutor {
        ...
        async fn execute(&self, _p: &mut Project) -> MonoloopResult<()> {
            Err(MonoloopError::Pipeline("intentional fail".into()))
        }
    }
