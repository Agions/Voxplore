//! monoloop-pipeline · PipelineService (运行时编排门面)
//!
//! ## 职责
...
use crate::{MonoloopResult, Pipeline, PipelineEvent, PipelineState, Project, StepStatus};
...
impl PipelineService {
    ...
    pub async fn start(&self, project: Project, deps: Arc<PipelineDeps>) -> MonoloopResult<()> {
        if self.is_running() {
            return Err(crate::MonoloopError::Pipeline("流水线正在运行中".into()));
        }
