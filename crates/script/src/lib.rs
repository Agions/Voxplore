//! monoloop-llm v1.0.0  · LlmProvider trait + 11 provider 真实实现 + LlmManager 故障切换
//!
//! ## 11 个 Provider
...
pub use monoloop_core::error::{LlmProviderKind, MonoloopError, MonoloopResult};
...
pub trait LlmProvider: Send + Sync {
    ...
    async fn chat(&self, req: &LlmRequest) -> MonoloopResult<LlmResponse>;
}
...
impl LlmProvider for OpenAiCompatible {
    ...
    async fn chat(&self, req: &LlmRequest) -> MonoloopResult<LlmResponse> {
        ...
        let api: ApiResp = resp.json().await.map_err(|e| MonoloopError::Llm {
            provider: self.kind,
...
pub fn local(base_url: Option<String>) -> OpenAiCompatible {
    OpenAiCompatible::new(
        ...
        "no-key-required",
        base_url.unwrap_or_else(|| "http://localhost:11434/v1/chat/completions".into()),
        "llama3.2",
    )
}
...
impl LlmManager {
    ...
    pub async fn chat(&self, req: &LlmRequest) -> MonoloopResult<LlmResponse> {
        ...
        let mut last_err: Option<MonoloopError> = None;
        ...
        Err(last_err.unwrap_or_else(|| {
            MonoloopError::Other("LlmManager: chain empty after iteration".into())
        }))
    }
}
