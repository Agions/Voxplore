//! monoloop-update v1.0.0  · 应用更新器 (M4 完整实装)
//!
//! ## 责任范围
...
/// - 仓库 / tag 模板可配置(默认 `qingshanyanyu/Monoloop`)
...
impl UpdateService {
    ...
    pub fn new(current_version: impl Into<String>, repo: impl Into<String>) -> Self {
        ...
        Self {
            ...
            client: reqwest::Client::builder()
                ...
                .user_agent("monoloop-updater/3.0")
                .build()
    ...
    /// 下载 latest 到 `dest`(可显式传;否则落到 std::env::temp_dir()/monoloop-update-<version>)
    pub async fn download(&self, dest: Option<PathBuf>) -> Result<PathBuf, UpdateError> {
        ...
        let dest = dest.unwrap_or_else(|| {
            std::env::temp_dir().join(format!("monoloop-update-{}.bin", info.version))
        });
...
mod tests {
    ...
    fn phase_state_default_is_idle() {
        let svc = UpdateService::new("1.0.0", "Agions/monoloop");
