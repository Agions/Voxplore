//! monoloop-core · ServiceContainer
//!
...
impl ServiceContainer {
    ...
    /// 解析一个服务 (返回克隆的 Arc)。
    /// 未注册返回 `MonoloopError::Config`。
    pub async fn resolve<T: Any + Send + Sync>(&self) -> Result<Arc<T>, super::MonoloopError> {
        let id = TypeId::of::<T>();
        let map = self.services.lock().await;
        map.get(&id)
            .and_then(|s| s.clone().downcast::<T>().ok())
            .ok_or_else(|| {
                super::MonoloopError::Config(format!(
