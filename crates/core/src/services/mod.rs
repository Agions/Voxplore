//! monoloop-core · 默认 19 个核心服务的注册与实现
//!
//! ## M2 已落地 (3 个)
...
pub async fn register_default_services(container: &ServiceContainer) {
    ...
    tracing::info!(
        "monoloop-core: 3/{total} default services registered (M2 milestone)",
        total = 19
    );
}
...
mod tests {
    ...
    async fn project_service_recent_works() {
        ...
        let path_a = dir.join("test_monoloop_a.monoloop");
        let path_b = dir.join("test_monoloop_b.monoloop");
