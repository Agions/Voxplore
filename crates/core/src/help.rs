//! monoloop-core · help — 帮助文档与 FAQ 注册表
...
Monoloop 是一款专为全自动第一人称短视频解说打造的桌面级 AI 创作系统。本指南将引导您在 3 分钟内完成首个解说视频的制作。
...
- 自动备份工程为 `.monoloop.json`，方便随时重修编辑。
"#.into(),
...
Monoloop 搭载了基于神经网络的 24kHz 高保真语音合成引擎与零样本（Zero-Shot）音色克隆模块。
...
Monoloop 的自动对齐引擎能够自动将合成的语音脚本与视频镜头时间轴进行毫秒级匹配。
...
"#.into(),
        ...
        reg.register(HelpTopic {
            ...
            title: "📖 `.monoloop.json` 工程结构与数据契约规范".into(),
            summary: Some("Monoloop 核心工程文件格式规范、轨道契约与本地 JSON 模式描述。".into()),
            category: HelpCategory::Reference,
            keywords: vec!["工程文件".into(), "JSON".into(), "Schema".into(), "数据契约".into()],
            content: r#"# `.monoloop.json` 工程文件规范描述

Monoloop 的工程文件采用轻量且跨平台兼容的 JSON 数据格式保存。每个解说工程对应一个独立的 `.monoloop.json` 结构。
...
2. **macOS 隐私与安全性授权**
   ...
   - 确保给 **Monoloop** 勾选了「文稿」与「Downloads」读取权限。
...
### Q1: Monoloop 会上传我的视频素材到云端服务器吗？
**答**: 绝对不会。Monoloop 是一款 100% 本地桌面软件。您的所有视频文件、帧分析、语音音频以及生成的字幕均仅保存在您本地的电脑磁盘上。

### Q2: 软件可以在断网（无网络连接）的环境下使用吗？
**答**: 可以。智能拆条、FFmpeg 剪辑、TTS 语音合成与导出渲染均支持完全离线运行。

### Q3: 应用需要用户登录或注册账号吗？
**答**: 不需要。Monoloop 坚守无登录设计原则（No-Login Policy），打开即用，无任何个人数据收集。
"#.into(),
...
**答**: 支持。Monoloop 底层集成了 FFmpeg 7.0+ 引擎，在处理 HDR10 / Dolby Vision 素材时，会自动色彩映射（Color Tone-mapping）至 SDR 标准 SDR BT.709，防止导出后色彩偏灰发白。
"#.into(),
...
### Q2: 多核 CPU 资源如何全速调用？
**答**: Monoloop 基于 Tokio 异步多线程架构，默认会自动分配所有物理 CPU 核心参与音视频并发编解码。
"#
            .into(),
            related: vec!["trouble-ffmpeg-cuda".into(), "guide-quick-start".into()],
        });
