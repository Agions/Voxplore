...
/// 全应用统一 Result 别名。
pub type MonoloopResult<T> = Result<T, MonoloopError>;
...
/// LLM 提供商标识 (与 `monoloop-llm::LlmManager` 中定义的 11 个 provider 一一对应)。
...
// 把 MonoloopError 序列化为 JSON 对象给 Tauri 前端
impl Serialize for MonoloopError {
    fn serialize<S: serde::Serializer>(&self, s: S) -> Result<S::Ok, S::Error> {
        ...
        let mut st = s.serialize_struct("MonoloopError", 2)?;
...
mod tests {
    ...
    fn serialization_shape() {
        let err = MonoloopError::Config("missing api_key".into());
